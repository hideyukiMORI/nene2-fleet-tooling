...
🔴 **これは機能の回復であって、意匠の変更ではない。** 狭い行でラベルが折り返し、**CJK は単語境界に守られない**ので
**「あなた」が1文字ずつ縦に積まれ**、バッジの高さが 26.5px → 43px になっていた
（nene-deal の W1 目視束を施主が見て出た**唯一の NG**）。

| | 0.19.0 | 0.19.1 |
| --- | --- | --- |
| **収まっているバッジ** | 35.88 × 26.5 | **35.88 × 26.5（不変）** |
| **vault の実文言・広い行 375px** | 50 × 25.39 | **50 × 25.39（不変）** |
| 狭い flex 行 180px | 🔴 49.2 × **43**（折れる） | ✅ 51 × 26.5 |
| block 容器 50px | 🔴 50 × **43** | ✅ 51 × 26.5 |

⇒ **変わるのは「いま折れている箇所」だけで、そこは今も壊れている。** スロットは増えていない。

#### 🔬 穴は2つ同時に開いていた

**ラベルが「折り返せる」（`white-space: normal`）ことと、バッジが「縮められる」（`flex-shrink: 1`）こと。**
**どちらか片方だけでも失敗が出る** ⇒ **片方を直すと、もう片方が開いたまま緑になる。**

**`whitespace-nowrap` を採った**（`shrink-0` ではなく）:
**`shrink-0` はその要素が flex 項目のときしか効かない** — 50px の block 容器で実測すると
`shrink-0` は依然として折れた（50 × 43）が、`nowrap` は折れなかった（51 × 26.5）。
`nowrap` は**縮みも塞ぐ**（flex 項目の自動最小サイズは min-content 幅で、`nowrap` はそれをラベル全体にする）。
⇒ **1クラスで両方の穴を、文脈に依らず塞ぐ。**

⚠️ **陰性対照を先に取ること。** 初回の測定は**容器が広すぎて対照が壊れず**、比較が無効だった。
**対照が壊れる幅を探してから**測っている（flex 180px / block 50px）。

#### line-height は潰れない

`nowrap` は `white-space` なので、艦が `leading-*` を `className` で持っていても**競合しない**
〔vault の構成で実測: 前後とも `line-height: 15.4px`〕。

**各艦でやること**: 0.19.0 → 0.19.1 は caret 内（`^0.19.0` が含む）。**lock が止めるので
`npm update @hideyukimori/nene2-ui`**。
⚠️ **長い文言のバッジを狭い容器に置いている艦は、そこが折れなくなる**（改善方向だが**見た目は変わる**）。

### `@hideyukimori/nene2-ui` 0.19.0（**minor — feat**・#463・**既定の描画が変わる**）

🔴 **載せ替える前に読むもの: この節全部。** 0.18.x までの追加と違い、**この版は既定の描画が変わります。**

| | 0.18.1 | 0.19.0 |
| --- | --- | --- |
| `Badge` の `gap` | **未指定**（アイコンと文字がくっつく） | `var(--spacing-x-3xs)` = **4px** |
| `Badge` の `font-size` | **継承**（周囲の本文と同じ） | `var(--text-x-2xs)` = **11px** |
| `Badge` の `font-weight` | **継承** | `var(--font-weight-semibold)` = **600** |

**新設スロット3本**（すべてスケール参照）:
`--spacing-x-slot-badge-gap` ／ `--text-x-slot-badge` ／ `--font-weight-x-slot-badge`

#### 🔴 なぜ「既定値不変」にできなかったか

**現行の既定が *未指定* なので、スロットを作ること自体が既定を決めます。** ⇒ **不変性を問う先が存在しない。**
受入基準は **「艦が取り戻せること」** に置き換えて検収しました（#470・実測済み）。

🔑 **キットの既定を使っていた艦は 0 でした**〔実測〕。`Badge` の消費者は `nene-vault` と `nene-deal` の2艦で、
**2艦とも独立に同じ3つを `className` で補っていました**（vault の `BADGE_CHROME` = `text-2xs font-semibold … gap-1.5`）。
⇒ **この版が置き換える描画は、どの消費者も使っていなかったもの。**

#### 🔴 `className` で補うのは、もう当てにしないでください

`cx` の docstring は「caller's classes come last so Tailwind's later-wins cascade lets them override」と
書いていますが、**これは誤りです**（#469）。**CSS では `class` 属性の順序は優先度を決めません。**
同一詳細度なら**生成 CSS 内の出現順**で決まり、実測では**キットの `BASE_CLASS` が3項目とも勝ちました**。

🟢 **スロットの再定義は確実に効きます**〔実測: 3項目とも艦の値が勝つ〕。変数そのものを差し替えるので生成順に依存しません。

#### 各艦でやること — 🔴 **「3つとも再定義せよ」ではありません**

**自艦の現行値を測り、既定と違うものだけ再定義してください。** 既定と同値なら**書かないのが正しい**
（不要な行は次に読む人に「ここは特別だ」と誤解させます）。

```bash
# 自艦のバッジの現行値を計算済みスタイルで測る
#   gap / font-size / font-weight を既定（4px / 11px / 600）と突き合わせる
```

```css
/* 違うものだけ、自艦のテーマで */
@theme {
  --spacing-x-slot-badge-gap: var(--spacing-x-2xs); /* 例: 8px にしたい場合 */
}
```

🔴 **リテラルではなくスケール参照で書いてください**（`6px` のような直書きは、スロットが存在する意味を消します。
消費側で `slot-values` 相当の検査を持っている艦は実際に落ちます — nene-vault が本件で踏みました）。

⚠️ **スケールに無い値は表現できません。** `--spacing-x-*` は **4 / 8 / 12 / 16 / 20 / 24 / 32 / 48px** の8段で、
**vault の 6px も deal の 5px も段ではありませんでした**。⇒ **移行は drift を段に載せ替える作業**です。
**見た目が変わるので、目視の確認に乗せてください。**

**版の上げ方**: 0.x の caret は minor を固定します ⇒ `^0.18.0` は 0.19.0 を**含みません**。
`^0.19.0` へ書き換え → `npm update @hideyukimori/nene2-ui` → `npm ls` で実インストール版 → 実ブラウザ。
🔴 **publish 前に `^0.19.0` を書かないでください**（`npm i` が解決できません。vault の指摘で判明）。

#### 消費艦の状況〔2026-08-26 20:3x 実測〕

- **nene-vault**: 🟢 段階1 完了（`f52c200`・`gap` のスロット行のみ）。**不活性と裏取り済み**（入っている 0.17.1 の dist は当該スロットを 0箇所参照・陽性対照 `x-slot-badge-pad-x` は 1箇所）。`font-size` / `weight` は**既定と同値なので再定義不要**
- **nene-deal**: W1 載せ替え中。この版を待っている

⚠️ **「npm に出た」と「艦の本番に載った」は別に数えてください。** vault は**見える変更なので次のリリース波に含める**（vault #481）。

### `@hideyukimori/nene2-ui` 0.18.1（**patch — doc only**・#389 / #467）

**載せ替える前に読むもの: 無し。実行時の挙動は 0.18.0 と同一。**

🔴 **なぜ patch を切ったか** — `files` は `["dist", "themes"]` なので **README と `themes/default.css` は
tarball に同梱される**。#389 の修正（`2a6c2ff`）が **0.18.0 のタグより後**に入ったため、
**公開済み 0.18.0 の README には幻のスロットが 2 件残っていた**〔`npm pack` で実物を落として grep・実測〕。
うち1件は **§② の旗艦例**＝「これを再定義しろ」の最初の例で、**そのスロットは存在しない**
（`Card` の padding は `pad` prop 由来）。README のとおりに書いた消費者には**何も起きない**。

🔑 **「版は宣言であって出荷の証拠ではない」の裏返し。** 修正が main に在っても、
**タグより後なら出荷物には入っていない。** 版の文字列を見る確認は**どちらの向きも素通りする**。
⇒ **doc しか変えていなくても、その doc が `files` に入っているなら publish が要る。**

| 出荷ファイル | 差分 | 中身 |
| --- | ---: | --- |
| `README.md` | 22行 | §② の旗艦例を実在スロットへ／主張を条件つきに／layout 部品は prop 由来と明記／`login-form` を製品側と明示 |
| `themes/default.css` | 13行 | ② 節**コメント**に在った同じ幻のスロット |
| `dist/index.js` | 4行 | **コメントのみ**（`tsc` が出力に残す）。`diff` で確認済み |

🟢 **コード・型・クラス文字列の変更は 0。** スロットの増減も無い（0.18.0 の 7本のまま）。

**各艦でやること**: 0.18.0 → 0.18.1 は caret 内（`^0.18.0` が含む）。**lock が止めるので
`npm update @hideyukimori/nene2-ui`**。ただし**実行時は何も変わらない**ので、
**急いで上げる必要は無い**（次に触るときで足りる）。README を読むなら上げてから。
