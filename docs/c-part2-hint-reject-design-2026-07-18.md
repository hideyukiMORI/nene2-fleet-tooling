...
4. **published と main の map 挙動乖離の扱い**（記録として確定・裁定不要）: C part-1（`6c6cc36`）が tokens 1.1.0 publish 後にマージされたため、npm published 1.1.0 と fleet main の 1.1.0 が**同一 `CODEMOD_MAP_VERSION` で別挙動**（published＝未知 namespace を silent x-送り＝`gap-x-stack` 衝突／main＝loud reject）。→ 本 C part-2 束の **1.2.0 bump で吸収**し、release note に「1.1.0 は publish 後に挙動が変わった版番号（published=silent x-送り／main=reject）で、正本は 1.2.0」と明記する（origin W1 #300 実測・2026-07-21）。
...
3. `CODEMOD_MAP_VERSION` minor bump（1.1.0→1.2.0）・publish.md への挙動変更記載。**この bump は機能追加だけでなく、published tokens 1.1.0（silent x-送り＝衝突あり）と main の 1.1.0（C part-1 `6c6cc36` で loud reject・publish 後マージ）が同一版番号で別挙動になっている乖離を、版で区別する是正を兼ねる**（origin W1 #300 実測・2026-07-21）。
