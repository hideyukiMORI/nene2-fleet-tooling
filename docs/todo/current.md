...
> 🔴 **自分の PR の番号と状態は書かない**（open PR は**本数**まで・詳細は issue へリンク）。
> マージ前に書いた「マージ待ち」は、**マージの瞬間に自分で自分を古くする**（hub 裁定 2026-08-05・#254）。
...
- **自艦の open PR はここに書かない**（上記規約）。`gh pr list --state open` で見ること。
  ⚠️ 裁定は**本数までは許容**しているが、08-14 に本数と番号を書いて**マージの瞬間に stale 化**させたので、ここでは数え方へ寄せてある（裁定より厳しい側・hub へ提案済み）。
