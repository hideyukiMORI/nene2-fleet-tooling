...
| ② | 「@smoke がある / CI で走る / 落ちたら止まる」は別物 | **`05 §5`（CI ゲート一覧）** | **EX-7**（族の核心形・新設案） | 🟢 可（3層とも機械判定可） |
...
**族の核心形（fleet 案 = EX-7）。** invoice 自身が「**「導入≠実行」の亜種**」と書いている。
...
### 🔴 なぜ族の「核心形」に推すか（fleet 案）

族の他のメンバーが**特定の技術領域の実例**（トークン・依存・生成物・build 射程）であるのに対し、②は「**存在する / 走る / 止める**」という**3層そのもの**を名指ししている。族の定義（§1.1 of `-serve-`）を実務の受入条件へ翻訳した形なので、**族の入口に置くのが分かりやすい**。EX-7 として新設し、他メンバーから逆参照する案を出す。**hub レビュー待ち。**
...
| EX-7（②を族の核心形として新設）の採否・ID 確定 | #161 / PR #209 の族台帳へ合流（hub レビュー） |
