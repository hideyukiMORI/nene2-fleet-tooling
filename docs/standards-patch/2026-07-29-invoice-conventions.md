...
## 5.5 🔴 還流レーンの作法（hub 裁定 2026-07-31・本 patch で2度踏んだ）

本 patch を書く過程で、**issue だけを読んでいたら条文を誤って書いていた**ケースを2度踏んだ。作法として明文化する。

> **条文化のときは、issue で発見し、正本で確定する**（MUST）。
> **issue は正本のスナップショットであって正本ではない。**
>
> 「正本」に含まれるもの:
> 1. 起草側が置いた**ドラフト文書**（`_work/handoff-*-draft-*.md`）— 追記は**正本にしか残らない**
> 2. 🔴 **起草側の実装とそのコメント** — 条文の「終点」は実装のほうが新しいことがある

### 踏んだ実例2件（どちらも本日・本 patch）

| | 何が起きたか | issue だけ読んでいたら |
|---|---|---|
| **①** | ①の追記（`stylingWith()` の評価時 throw）が **#162 の issue 本文に無かった**。起草時に fleet が落ちており指示書方式へ切り替えられていたため、**正本にしか存在しなかった** | 条文が丸ごと1件欠ける |
| **②** | 起草側の**原文 ①手順2**（app config からの名前付き re-export）が、起草側の**現行実装とすでに食い違っていた**。追記の対処を入れた結果 `e2eConfig` は名前付き export されておらず、今日 `SyntaxError: does not provide an export named 'e2eConfig'` が実測された | **古い終点を条文に焼き付け**、写した艦が同じ穴を踏み直す |

②が示すのは、**正本のドラフトすら実装に遅れうる**ということ。**起草側の実装コメントまで読んで初めて終点が確定する**（invoice のリポ直下 config には失敗原因 (a)(b) が両方コメントで書かれていた — fleet はこれを読まずに (b) だけを見て、#211 で「A は G-7 逆行」という誤った評価を出した）。

### なぜ「気をつける」で終わらせないか

この2件はどちらも **relay / issue という装置が正常に動いた結果**である。①は relay が落ちたので指示書へ退避され（**出口は機能した**）、②は起草側が実装を進めた結果（**正しい進捗**）。**どこにも過失が無いのに条文は壊れる。** ＝ EX-2（記録は出口ではない）と同族で、「装置は動いていたが、次に読む人が最新に辿り着く経路が無い」形。
