...
| 不存在 rule-id | 49（missing 47・malformed 2） | **18** | 前版の測定後、配布 config 側にルールが追加された（W0a stage3 以降の PR 群）ため実際に解消した分 |
| [P] 列挙外タグ | 1 | **0** | 規約側で是正済み |
| MUST 総数 / タグ欠落 | 399 / 212 | **407 / 214** | 規約文書自体が加筆された（現状注記の追加等） |
...
| ファイル | sha256 |
|---|---|
| `README.md` | `4d9814348d79d1ed` |
| `01-architecture.md` | `30c0048963dbb50f` |
| `02-data-flow.md` | `06ee30d7305d5952` |
| `03-styling-theming.md` | `53d47dd024e90580` |
| `04-i18n.md` | `3aaa1c6a2099c4b5` |
| `05-enforcement-migration.md` | `650b54b9decedd8b` |

### (2) 入力 — 規約文書の出所コミット

| リポ | commit |
|---|---|
| `xi-workspace`（`_work/`） | `f0550ae6703d27e8ed15681583511f4f1c9f735c` |
...
| | |
|---|---|
| `nene2-fleet-tooling` | `06904b771f069242dea8b27d3c8c9081a149bad8` |
...
| 検査 | 判定 | red 内訳 |
|---|---|---|
| check:standards-doc | **red** | タグ欠落 MUST **214**／不存在 rule-id **18**／[P] 列挙外 **0** |
| check:exemplars | **red** | ユニーク [X] 8 件中 **4 失敗**（resolved 4・検査不能 0） |

- MUST 総数 **407**・機械強制カバレッジ率 **46.2%**（タグ付与率 47.4%）— SHOULD 目標 90%+／散文 MUST 上限 60 に対する**観測値**（G-5 補記 — 数値ゲート化 MUST NOT）。
...
- MUST 総数: **407**（タグ付き 193・タグ欠落 **214**）
- 機械強制カバレッジ率（[E][S][T][G][C][X] / MUST 総数）: **46.2%**（タグ付与率 [P] 込み: 47.4%）
- SHOULD 目標（観測のみ — G-5 補記・ゲート化 MUST NOT）: 機械強制 90%+ / 散文 MUST 上限 60（実測 407 — 上限比 超過）

### タグ欠落 MUST（214 件 — SHOULD 格下げ棚卸しリスト・G-1）
...
| 04-i18n.md | 169 | > **本節の exemplar 3件は現物が条文に未準拠であり、準拠は W1 の i18n パッケージ移行に同乗させる。それまで `check:exemplars` の red リスト（RAT-3(a)・AI-21）に載るのが正 — re |
| 04-i18n.md | 187 | - `SUPPORTED_LOCALES` への掲載は **parity 検査（I18N-20/21）100% 通過が条件**（MUST）。 |
| 04-i18n.md | 188 | - 通過しないロケールは**カタログファイルごと削除する**（MUST）。「キーだけ揃ったスタブ」「一部訳出」の恒久化 MUST NOT。 |
| 04-i18n.md | 203 | - 権威カタログは ja **のみ**。en を権威とする実装 MUST NOT。裁定根拠: 権威カタログ＝施主が正誤判定できる唯一のカタログ（en 権威では日本語訳の質を機械も施主もゲートできない）。en 支持の提出は R1 で 0 [R |
| 04-i18n.md | 232 | - 英語セマンティック・ドットパス **`domain.area.intent`**。全キーは次の正規表現に適合する（MUST）: |
| 04-i18n.md | 241 | - MUST NOT: 日本語キー（`'保存': …`）・連番キー（`msg1`, `label2`）・値をキーにする source-as-keys（Lingui 方式 — ドリフトの温床 [R1⑦・リサーチ §2]）・kebab-case |
| 04-i18n.md | 242 | - **vault の既存キーの扱い**（起草時実測: 384 キー中 256 が snake_case または `_meta.*` — 66.7%）: vaultJsonCatalog 公認差異（I18N-5 — 登録範囲は JSON 形 |
| 04-i18n.md | 258 | - プレースホルダは `{{lowerCamelCase}}` のみ。`{param}`（一重括弧）・`%s`・`${…}` MUST NOT。parity ヘルパが値文字列を走査し、一重括弧プレースホルダ・不正なパラメータ名で fail  |
| 04-i18n.md | 259 | - 同一キーのプレースホルダ集合は**全ロケールで一致** MUST（`{{name}}` が ja にあって en にない、は parity fail）。 |
| 04-i18n.md | 275 | - 複数形が要るメッセージは **CLDR カテゴリ接尾辞つきのキー族**で書く（MUST）: `<base>.other`（全ロケール必須）＋当該ロケールの CLDR カテゴリが要求する分だけ `.one` / `.few` / `.ma |
| 04-i18n.md | 304 | - 通貨・日付・数値・日時のユーザ提示文字列は `@hideyukimori/nene2-i18n/format` の関数**のみ**で生成する（MUST）。 |
| 04-i18n.md | 334 | 次の**列挙**に該当する文字列は必ず `t(MessageKey)`（または plural/format）経由とする（MUST）: |
| 04-i18n.md | 344 | 適用範囲は **`src/**` 全域**（除外の有限列挙は I18N-16）。features/pages 限定の適用 MUST NOT — 限定 lint の穴が field の `aria-label="閉じる"` 直書き 5 箇所（ |
| 04-i18n.md | 347 | - **機械強制の実効範囲の明記**（誠実性 — AM-19 の故障様式）: 本 MUST を機械強制するのは**日本語文字列の検出（I18N-16）**と**スターター/新規リポ限定の言語非依存 lint（I18N-19 — SHOULD |
| 04-i18n.md | 373 | - **検出プローブ（正例・負例）同梱 MUST**（AM-16 運営則: lint の昇格には検出プローブ添付・自己テストをパッケージテストに同梱）。プローブ 6 本 [AI-19]: |
| 04-i18n.md | 384 | - **lint 除外（ignore）の有限列挙**（この列挙以外の除外 MUST NOT — リポ側追加は gate-integrity FAIL [AM-11(iii)]）: `**/*.test.*` / `**/*.stories. |
| 04-i18n.md | 396 | - shared/ui コンポーネントへの表示文字列（aria-label 含む）内蔵 MUST NOT。ユーザ知覚文字列は required prop（例 `closeLabel: string`・デフォルト値 MUST NOT）で受け、 |
| 04-i18n.md | 417 | - スターターは UI 文字列の `t()` 化＋ja/en カタログ同梱を W0 完了条件に含める（MUST）。現物: スターターは英語ハードコード（`TagList.tsx` ほか）— JP 正規表現では構造的に検知不能 [AM-19] |
| 04-i18n.md | 418 | - **スターター・新規リポ限定**で、言語非依存のユーザ知覚文字列 literal 禁止 lint を有効化する（SHOULD — 既存リポへは偽陽性コスト未実測につき MUST を張らない: ⑨メタ規約の自己適用 [AM-19]）。 |
| 04-i18n.md | 424 | 検出は 4 層で行う。どれか 1 層に頼る実装 MUST NOT（各層が別の故障様式を塞ぐ）。 |
| 04-i18n.md | 428 | `locales.test.ts` は次の 1 呼び出しを含む（MUST — `gen` が吐く定型）: |
| 04-i18n.md | 450 | - **全ロケール対**で同一値のキーの比率を検査し、`maxIdenticalRatio`（v1 規定値 **20%**）超過で fail（MUST）。権威対限定 MUST NOT（「en の値を de/fr にコピー」経路に盲目 [R5 |
| 04-i18n.md | 455 | ### I18N-22: 沈黙フォールバック MUST NOT — DEV は可視化・本番のみ権威へ — [R1⑦] |
| 04-i18n.md | 457 | - DEV ビルド: 現在ロケールのカタログにキーが解決できない場合、`∅` ＋キー名を**そのまま描画**し（`∅invoice.list.empty`）、`console.error` を 1 キーにつき 1 回発火する（MUST）。 |
| 04-i18n.md | 458 | - 本番ビルド: 権威カタログ（ja）へフォールバックする（MUST — ユーザに `∅` を見せない唯一の場所）。 |
| 04-i18n.md | 509 | - アプリのルートに `@hideyukimori/nene2-i18n/react` の `I18nProvider` を 1 つ置く（MUST）。 |
| 04-i18n.md | 510 | - Provider は **scope 要素**に `lang` と `dir` を同期する（内蔵機能 — 手書き同期 MUST NOT）。既定 scope は `document.documentElement`（変更 MUST NOT |
| 04-i18n.md | 511 | - **widget 製品**（corpus / concierge — `<html>` を所有しない）は、⑥のテーマスコープと**同一の要素**を公認差異登録し、lang/dir をそこに設定する（MUST）。現物の実害: corpus |
| 04-i18n.md | 529 | - 現在ロケールはアプリグローバル状態（auth / theme / **locale** / toast — R1③の列挙）であり、nene2-i18n/react の `localeStore`（`useSyncExternalStor |
| 04-i18n.md | 530 | - 永続化は package が行う（v1 仕様: `localStorage` キー `nene2-locale` — ⑥章 DM-03 のテーマ保存キー `nene2-theme` と同一の命名規則。製品側からの直接読み書き MUST  |
| 04-i18n.md | 531 | - **初期ロケール解決も package の責務**（v1 仕様）: `I18nProvider` / `localeStore` が `保存値（nene2-locale）→ navigator.languages（SUPPORTED_L |
| 04-i18n.md | 535 | - CSS 論理プロパティ（`margin-inline-start` 等）による RTL 耐性は SHOULD（MUST に張れる強制タグがないため — [R1⑦]）。`dir` は I18N-23 が scope 要素に設定する。 |
| 04-i18n.md | 542 | - コンポーネント/フックのテストは `renderWithI18n(ui, { locale: 'ja' })`（generator が吐く）でロケールを **ja に固定**する（MUST）。ロケール未固定の文字列 expect MUS |
| 04-i18n.md | 543 | - 期待値は **ja カタログ値の直書き可**（MAY）— ja が権威カタログである以上、直書き期待値がカタログの実質検査になる [R3⑧裁定]。テストコードを `t()` 経由に強制すること MUST NOT（R3 REJECTED） |
| 04-i18n.md | 566 | 移行 DoD ベクトルに i18n キーを含める（styling 単独 DoD MUST NOT — [R3⑩]）。キー名の正本は W0 の conformance skeleton（dx 管轄）。v1 提案: |
| 04-i18n.md | 581 | - 1 PR = 1 リポ × 1 軸（P-3）。i18n 是正と transport 置換・token rename の同一 PR 混載 MUST NOT。 |
| 04-i18n.md | 582 | - vault の **key-grammar baseline**（I18N-10 — 256 キーの rename はバックエンド composer locales と同時変更のクロススタック）は vault の work order  |
| 04-i18n.md | 591 | 差異登録の理由は**外部制約由来のみ**・無期限登録 MUST NOT [R1⑨]。 |
...
### 不存在 rule-id（18 件 — RAT-2 実在照合 FAIL）
...
| 03-styling-theming.md | 192 | `[S:no-double-layer-import]` | `no-double-layer-import` | missing |
| 03-styling-theming.md | 468 | `[S:no-double-layer-import]` | `no-double-layer-import` | missing |
| 03-styling-theming.md | 747 | `[S:no-double-layer-import]` | `no-double-layer-import` | missing |
| 04-i18n.md | 332 | `[E:I18N-19 言語非依存 lint（未配布——スターター/新規リポ限定・SHOULD 水準・既存リポは偽陽性コスト未実測につき未展開。AM-19）]` | `I18N-19 言語非依存 lint` | missing |
| 04-i18n.md | 536 | `[E:jsx-a11y strict preset]` | `jsx-a11y strict preset` | missing |

### [P] 列挙外タグ（0 件 — G-3）

なし

補足: rule-id 照合 ok 88 件（うち prefix 補完解決 16 件）・構文プレースホルダのスキップ 7 件。[X:file#anchor] は 8 参照を check:exemplars へ委譲（G-2）。
...
| `[X:nene-payout/frontend/src/shared/i18n/locales.test.ts#nene2-exemplar:parity-test]` | anchor-missing | アンカーコメント `[nene2-exemplar:parity-test]` が nene-payout/frontend/src/shared/i18n/locales.test.ts に未植栽（origin/main（fetch 済み）） | 04-i18n.md:426 |
| `[X:nene-payout/frontend/src/shared/i18n/locales.ts#nene2-exemplar:locales-config]` | anchor-missing | アンカーコメント `[nene2-exemplar:locales-config]` が nene-payout/frontend/src/shared/i18n/locales.ts に未植栽（origin/main（fetch 済み）） | 04-i18n.md:143 |
| `[X:nene-records/frontend/src/shared/api/errors.ts#nene2-exemplar:problem-map]` | anchor-missing | アンカーコメント `[nene2-exemplar:problem-map]` が nene-records/frontend/src/shared/api/errors.ts に未植栽（origin/main（fetch 済み）） | 04-i18n.md:481 |
