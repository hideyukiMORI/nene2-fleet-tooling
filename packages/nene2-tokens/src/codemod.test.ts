...
describe('buildPlan — payout#159 現物での機械導出', () => {
  ...
  it('derives exactly the 12 token renames PR #159 measured (#17: font-weight 行のみ v1.0.2 で是正)', () => {
  ...
  // `--font-size-*` / `--line-height-*` は **v4 namespace ではない**（実測: utility を生成しない）
  // ため、上の表では従来どおり先頭セグメント直後に x- が入る。その「legacy 綴りを
  // v4 namespace へ再ホームするか（--font-size-body → --text-x-body 等）」は語彙判断なので
  // v1.0.2 のスコープ外 — #17 に選択肢を提示して据え置き。
  it('preserves the v4 namespace when inserting x- (#17)', () => {
    ...
    // 未知 namespace（v4 に無い）は先頭セグメント直後 — 従来挙動を保存する
    expect(classifyTokenName('--z-modal')).toEqual({ kind: 'rename', name: '--z-x-modal' });
  });
