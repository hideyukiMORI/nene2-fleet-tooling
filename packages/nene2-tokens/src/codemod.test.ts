...
describe('C part-2 — LEGACY_PREFIX_HINTS（hint 付き reject 表・#94/#125）', () => {
  it('font-size は既定 W1 では reject せず x-送り維持（activeFrom W3・§4-1 施主裁定）', () => {
    // 既定（activeWave 未指定＝W1）: 現行挙動を壊さない。
    expect(classifyTokenName('--font-size-body')).toEqual({
      kind: 'rename',
      name: '--font-x-size-body',
    });
    expect(classifyTokenName('--font-size-body', 'common', { activeWave: 'W1' }).kind).toBe(
      'rename',
    );
  });

  it('font-size は W3 で reject（step 6 の x-送りより前に止まる）＋hint は text', () => {
    const r = classifyTokenName('--font-size-body', 'common', { activeWave: 'W3' });
    expect(r.kind).toBe('reject');
    if (r.kind === 'reject') {
      expect(r.reason).toContain('--text-*'); // 正しい家
      expect(r.reason).toContain('--font-size-body');
      expect(r.reason).not.toContain('--font-x-size-body'); // x-送りしていない
    }
  });

  it("'now' 系（line-height/z/border-width）は W1 でも reject・hint を運ぶ", () => {
    const lh = classifyTokenName('--line-height-body');
    expect(lh.kind).toBe('reject');
    if (lh.kind === 'reject') expect(lh.reason).toContain('--leading-*');

    for (const bare of ['--z-modal', '--border-width-thin']) {
      const r = classifyTokenName(bare);
      expect(r.kind).toBe('reject');
      // hint 無し系は plain var へ誘導（§4-3(a)）
      if (r.kind === 'reject') expect(r.reason).toContain('plain var');
    }
  });

  it('§5(b): LEGACY_PREFIX_HINTS の prefix は v4 実在 namespace と素（実在を B1 に書けない）', () => {
    const v4 = new Set(TAILWIND_V4_NAMESPACES);
    for (const prefix of Object.keys(LEGACY_PREFIX_HINTS)) {
      expect(v4.has(prefix), `${prefix} は v4 namespace に載っている`).toBe(false);
    }
  });

  it('§5: longest-match 網羅 — multi-segment namespace はその prefix より前に並ぶ', () => {
  ...
  it('B1 に該当しない既知 namespace は従来どおり x-送り（表の副作用が無い）', () => {
    // font-weight は v4 実在 namespace＝B1 非該当・step 6 で x-送り（#17 の保存）。
    expect(classifyTokenName('--font-weight-medium')).toEqual({
      kind: 'rename',
      name: '--font-weight-x-medium',
    });
