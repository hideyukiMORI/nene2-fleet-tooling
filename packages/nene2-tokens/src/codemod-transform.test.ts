...
// C part-1（#92）: 現物 theme の `--line-height-body` は v4 namespace 外＝buildPlan が
// loud stop する（codemod.test.ts で固定）。ここは class 置換の検証なので除いて plan を作る。
...
describe('codemod transform — payout#159 現物の行を再現する', () => {
  ...
  // NOTE(#17): `font-medium → font-x-medium` は map v1.0.2 で**新たに**置換されるようになった。
  // v1.0.1 までは roots['font-weight'] 未登録のため silent skip で `font-medium` のまま残り、
  // テーマ側だけ改名されて Tailwind 既定値へ無言フォールバックしていた。
  // `font-sans`/`text-body` が不変なのは、対応する `--font-family-*`/`--font-size-*` が
  // v4 の namespace ではなく utility を生成しない（＝元から silent no-op）ため — #17 の据え置き分。
  it('rewrites the accent-contrast button (payout: text-accent-contrast → text-on-accent)', () => {
    ...
    expect(run(src)).toBe(
      `<button className="rounded-x-md bg-accent px-x-inline-md py-x-stack-sm font-sans text-body font-x-medium text-on-accent">go</button>;\n`,
    );
  });
...
describe('codemod transform — prettier 固定点（splice 方式の眼目・実証）', () => {
  ...
  it('codemod 出力は pinned prettier の固定点のまま（payout 現物・実測）', async () => {
    ...
    // 6 = v1.0.1 までの 5 ＋ `font-medium → font-x-medium`（#17 で silent skip を是正した分）。
    // 置換が 1 つ増えても payout 現物の prettier 固定点は保たれる（下の check が実測）。
